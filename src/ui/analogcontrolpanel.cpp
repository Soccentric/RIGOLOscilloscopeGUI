...
void AnalogControlPanel::setupUI() {
  ...
  connect(m_enableBtn, &QPushButton::toggled, this,
          &AnalogControlPanel::onChannelEnableToggled);
  ...
  for (int i = 0; i < 4; ++i) {
    ...
    btn->setStyleSheet(
        QString("background-color: %1; color: #000;").arg(channelColors[i]));
    connect(btn, &QPushButton::clicked, this,
            [this, i]() { m_channelCombo->setCurrentIndex(i); });
  ...
  connect(m_scaleDial, &QDial::valueChanged, this,
          &AnalogControlPanel::onScaleDialChanged);
  ...
  connect(m_offsetSlider, &QSlider::valueChanged, this,
          &AnalogControlPanel::onOffsetSliderChanged);
  offsetLayout->addWidget(m_offsetSlider);

  // Center button
  m_centerBtn = new QPushButton("Center");
  m_centerBtn->setObjectName("actionBtn");
  connect(m_centerBtn, &QPushButton::clicked, this,
          [this]() { m_offsetSlider->setValue(0); });
  ...
  connect(m_triggerDial, &QDial::valueChanged, this,
          &AnalogControlPanel::onTriggerDialChanged);
...
void AnalogControlPanel::applyStyles() {
  ...
  // We only need to set specific object names for global styling if needed
}

void AnalogControlPanel::refresh() { updateChannelControls(); }

QString AnalogControlPanel::selectedChannel() const { return m_currentChannel; }
  ...
  if (m_channels.contains(m_currentChannel)) {
    ...
    if (m_connection && m_connection->isConnected()) {
      m_connection->sendCommand(
          SCPI::channelDisplay(m_currentChannel, enabled));
    }
  }
}
...
void AnalogControlPanel::onCouplingChanged(int index) {
  ...
  if (m_connection && m_connection->isConnected()) {
    ...
    m_connection->sendCommand(
        SCPI::channelCoupling(m_currentChannel, couplingStr));
  }
}
...
void AnalogControlPanel::updateChannelControls() {
  ...
  m_offsetValueLabel->setText(
      QString("%1 V").arg(channel->offset(), 0, 'f', 2));
