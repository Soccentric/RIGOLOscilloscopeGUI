...
MainWindow::MainWindow(QWidget *parent)
  ...
  // Apply classic theme to window
  // setStyleSheet is now handled globally by ThemeManager

  // Create waveform provider after connection
  m_waveformProvider =
      std::make_unique<WaveformProvider>(m_connection.get(), this);
  ...
  for (int i = 1; i <= 4; ++i) {
    ...
    auto *channel =
        new ScopeChannel(name, ScopeChannel::ChannelType::Analog, this);
    channel->setColor(QColor(channelColors[i - 1]));
    if (i == 1)
      channel->setEnabled(true); // Enable CH1 by default
  ...
  if (m_connection->isConnected()) {
    QMessageBox::StandardButton reply =
        QMessageBox::question(this, "Confirm Exit",
                              "You are still connected to the oscilloscope. Do "
                              "you want to disconnect and exit?",
...
void MainWindow::createActions() {
  ...
  connect(m_screenshotAction, &QAction::triggered, this,
  ...
  connect(m_resetLayoutAction, &QAction::triggered, this,
  ...
  connect(m_toggleLeftPanelAction, &QAction::triggered, this,
          &MainWindow::onToggleLeftPanel);
  ...
  connect(m_toggleRightPanelAction, &QAction::triggered, this,
          &MainWindow::onToggleRightPanel);
  ...
  connect(m_toggleFFTDisplayAction, &QAction::triggered, this,
          &MainWindow::onToggleFFTDisplay);
  ...
  connect(m_autoScaleAction, &QAction::triggered, this,
          &MainWindow::onAutoScale);

  m_forceTriggerAction = new QAction(tr("&Force Trigger"), this);
  m_forceTriggerAction->setShortcut(QKeySequence(Qt::Key_F));
  connect(m_forceTriggerAction, &QAction::triggered, this,
          &MainWindow::onForceTrigger);
  ...
  connect(m_channel1Action, &QAction::triggered, this,
          [this]() { toggleChannel("CH1"); });
  ...
  connect(m_channel2Action, &QAction::triggered, this,
          [this]() { toggleChannel("CH2"); });
  ...
  connect(m_channel3Action, &QAction::triggered, this,
          [this]() { toggleChannel("CH3"); });
  ...
  connect(m_channel4Action, &QAction::triggered, this,
          [this]() { toggleChannel("CH4"); });
  ...
  connect(m_protocolDecoderAction, &QAction::triggered, this,
          &MainWindow::onOpenProtocolDecoder);

  m_mathChannelAction = new QAction(tr("&Math Channel"), this);
  m_mathChannelAction->setShortcut(QKeySequence(Qt::CTRL | Qt::Key_M));
  connect(m_mathChannelAction, &QAction::triggered, this,
          &MainWindow::onOpenMathChannel);
...
void MainWindow::createCentralLayout() {
  ...
  // === HEADER ===
  m_headerWidget =
      new HeaderWidget(m_connection.get(), m_scopeSettings.get(), this);
  ...
  // === FOOTER ===
  m_footerWidget =
      new FooterWidget(m_channels, m_measurementEngine.get(), this);
...
void MainWindow::setupConnections() {
  ...
  connect(m_connection.get(), &DeviceConnection::connectionStateChanged, this,
          [this](DeviceConnection::ConnectionState state) {
            onConnectionStateChanged(static_cast<int>(state));
          });

  connect(m_connection.get(), &DeviceConnection::deviceIdChanged, this,
          &MainWindow::onDeviceIdReceived);

  // Waveform updates
  connect(m_waveformProvider.get(), &WaveformProvider::waveformDataReady, this,
          [this](const QString &channel, const QVector<QPointF> &data) {
  ...
  connect(m_headerWidget, &HeaderWidget::connectRequested, this,
          &MainWindow::onConnectAction);
  connect(m_headerWidget, &HeaderWidget::disconnectRequested, this,
          &MainWindow::onDisconnectAction);
  connect(m_headerWidget, &HeaderWidget::runModeChanged, this,
          &MainWindow::onRunModeChanged);
  connect(m_headerWidget, &HeaderWidget::timebaseChanged, this,
          &MainWindow::onTimebaseChanged);

  // Analog panel connections
  connect(m_analogPanel, &AnalogControlPanel::channelChanged, this,
          &MainWindow::onChannelSettingsChanged);
  connect(m_analogPanel, &AnalogControlPanel::scaleChanged, this,
          [this](const QString &channel, double) {
            onChannelSettingsChanged(channel);
          });
  connect(m_analogPanel, &AnalogControlPanel::offsetChanged, this,
          [this](const QString &channel, double) {
            onChannelSettingsChanged(channel);
          });
  connect(m_analogPanel, &AnalogControlPanel::triggerLevelChanged, this,
          &MainWindow::onTriggerLevelChanged);

  // Digital panel connections
  connect(m_digitalPanel, &DigitalControlPanel::runStopClicked, this,
          &MainWindow::onRunStop);
  connect(m_digitalPanel, &DigitalControlPanel::singleClicked, this,
          &MainWindow::onSingle);
  connect(m_digitalPanel, &DigitalControlPanel::autoScaleClicked, this,
          &MainWindow::onAutoScale);
  connect(m_digitalPanel, &DigitalControlPanel::forceTriggerClicked, this,
          &MainWindow::onForceTrigger);
  connect(m_digitalPanel, &DigitalControlPanel::clearClicked, this,
          &MainWindow::onClear);
  connect(m_digitalPanel, &DigitalControlPanel::fftClicked, this,
          &MainWindow::onOpenFFT);
  connect(m_digitalPanel, &DigitalControlPanel::protocolDecoderClicked, this,
          &MainWindow::onOpenProtocolDecoder);
  connect(m_digitalPanel, &DigitalControlPanel::measurementAdded, this,
          &MainWindow::onMeasurementAdded);
...
void MainWindow::onExportData() {
  QString filter =
      tr("CSV Files (*.csv);;Binary Files (*.bin);;MATLAB Files (*.mat)");
  QString filename = QFileDialog::getSaveFileName(
      this, tr("Export Waveform Data"), m_lastExportPath, filter);
...
void MainWindow::onTakeScreenshot() {
  ...
  QString filename = QFileDialog::getSaveFileName(
      this, tr("Save Screenshot"), m_lastScreenshotPath,
...
void MainWindow::onAbout() {
  QMessageBox::about(
      this, tr("About RIGOL Oscilloscope Pro"),
      tr("<h2>RIGOL Oscilloscope Pro</h2>"
         "<p>Version 2.0.0</p>"
         "<p>A professional-grade oscilloscope control application for RIGOL "
         "MSO/DS series.</p>"
         "<p><b>Features:</b></p>"
         "<ul>"
...
void MainWindow::onCheckForUpdates() {
  QDesktopServices::openUrl(
      QUrl("https://github.com/rigol-oscilloscope-gui/releases"));
}
...
void MainWindow::updateStatusBar() {
  if (m_connection->isConnected()) {
    ...
    if (m_isRunning) {
      m_footerWidget->setAcquisitionProgress((m_acquisitionCount % 100));
    }
  }
}
...
void MainWindow::onMeasurementsUpdated() { m_digitalPanel->refresh(); }
...
void MainWindow::toggleChannel(const QString &channelName) {
  ...
  if (channelName == "CH1")
    m_channel1Action->setChecked(enabled);
  else if (channelName == "CH2")
    m_channel2Action->setChecked(enabled);
  else if (channelName == "CH3")
    m_channel3Action->setChecked(enabled);
  else if (channelName == "CH4")
    m_channel4Action->setChecked(enabled);
...
void MainWindow::onMeasurementAdded(const QString &type,
                                    const QString &channel) {
  // Convert type string to enum
  MeasurementType measType = MeasurementType::Vpp;

  if (type == "Vpp")
    measType = MeasurementType::Vpp;
  else if (type == "Vmax")
    measType = MeasurementType::Vmax;
  else if (type == "Vmin")
    measType = MeasurementType::Vmin;
  else if (type == "Vavg")
    measType = MeasurementType::Vavg;
  else if (type == "Vrms")
    measType = MeasurementType::Vrms;
  else if (type == "Frequency")
    measType = MeasurementType::Frequency;
  else if (type == "Period")
    measType = MeasurementType::Period;
  else if (type == "Rise Time")
    measType = MeasurementType::RiseTime;
  else if (type == "Fall Time")
    measType = MeasurementType::FallTime;
  else if (type == "Duty Cycle")
    measType = MeasurementType::DutyCycle;
