...
void WaveformDisplay::zoomToFit()
{
    ...
    for (auto *channel : m_channels) {
        if (!channel->isEnabled() || channel->data().isEmpty()) continue;
...
void WaveformDisplay::resizeGL(int w, int h)
{
    glViewport(0, 0, w, h);
}
...
void WaveformDisplay::drawGrid()
{
    ...
    for (int i = 1; i < m_gridHorizontal; ++i) {
        ...
        } else if (m_gridStyle == 1) {
            // Dots
            double divH = plotH / static_cast<double>(m_gridVertical);
            for (int j = 0; j <= m_gridVertical; ++j) {
                int y = margin + static_cast<int>(j * divH);
    ...
    for (int i = 1; i < m_gridVertical; ++i) {
        ...
        if (m_gridStyle == 2) {
            ...
            for (int j = 0; j <= m_gridHorizontal; ++j) {
                int x = margin + static_cast<int>(j * divW);
                painter.drawPoint(x, y);
            }
        }
    }
    ...
    QPen centerPen(m_gridColor.lighter(130), 1);
...
{
    if (!channel || channel->data().isEmpty()) return;
    ...
    for (const QPointF &p : data) {
        ...
        if (x < margin - 10 || x > margin + plotW + 10) continue;
...
void WaveformDisplay::drawCursors()
{
    ...
    if (type == CursorManager::CursorType::Horizontal || 
        ...
        double x1 = margin + (m_cursorManager->cursorX1() - m_xMin) * plotW / (m_xMax - m_xMin);
        double x2 = margin + (m_cursorManager->cursorX2() - m_xMin) * plotW / (m_xMax - m_xMin);
        ...
        QString x1Text = QString("X1: %1").arg(formatTime(m_cursorManager->cursorX1()));
        QString x2Text = QString("X2: %1").arg(formatTime(m_cursorManager->cursorX2()));
        QString dxText = QString("ΔX: %1").arg(formatTime(m_cursorManager->deltaX()));
        QString freqText = QString("1/ΔX: %1").arg(formatFrequency(m_cursorManager->frequency()));
    ...
    if (type == CursorManager::CursorType::Vertical || 
        ...
        double y1 = margin + plotH - (m_cursorManager->cursorY1() - m_yMin) * plotH / (m_yMax - m_yMin);
        double y2 = margin + plotH - (m_cursorManager->cursorY2() - m_yMin) * plotH / (m_yMax - m_yMin);
        
        painter.drawLine(QPointF(margin, y1), QPointF(margin + plotW, y1));
        painter.drawLine(QPointF(margin, y2), QPointF(margin + plotW, y2));
        
        // Labels
        QString y1Text = QString("Y1: %1 V").arg(m_cursorManager->cursorY1(), 0, 'f', 3);
        QString y2Text = QString("Y2: %1 V").arg(m_cursorManager->cursorY2(), 0, 'f', 3);
        QString dyText = QString("ΔY: %1 V").arg(m_cursorManager->deltaY(), 0, 'f', 3);
...
void WaveformDisplay::drawTrigger()
{
    ...
    double y = margin + plotH - (m_triggerLevel - m_yMin) * plotH / (m_yMax - m_yMin);
    ...
    if (y >= margin && y <= margin + plotH) {
        ...
        painter.drawLine(margin, static_cast<int>(y), margin + plotW, static_cast<int>(y));
    }
...
void WaveformDisplay::drawLabels()
{
    ...
    font.setBold(true);
    painter.setFont(font);
    painter.drawText(margin, 20, QString("Time: %1/div").arg(formatTime(m_timePerDiv)));
...
void WaveformDisplay::drawChannelLabels()
{
    ...
    for (auto *channel : m_channels) {
        ...
        QString label = QString("%1: %2 V/div")
                        .arg(channel->name())
                        .arg(channel->scale());
...
QString WaveformDisplay::formatTime(double seconds) const
{
    ...
    else if (absSeconds >= 1) return prefix + QString::number(absSeconds, 'f', 3) + " s";
    else if (absSeconds >= 1e-3) return prefix + QString::number(absSeconds * 1e3, 'f', 3) + " ms";
    else if (absSeconds >= 1e-6) return prefix + QString::number(absSeconds * 1e6, 'f', 3) + " µs";
    else return prefix + QString::number(absSeconds * 1e9, 'f', 3) + " ns";
}

QString WaveformDisplay::formatFrequency(double hz) const
{
    if (hz == 0) return "0 Hz";
    else if (hz >= 1e9) return QString::number(hz / 1e9, 'f', 3) + " GHz";
    else if (hz >= 1e6) return QString::number(hz / 1e6, 'f', 3) + " MHz";
    else if (hz >= 1e3) return QString::number(hz / 1e3, 'f', 3) + " kHz";
    else return QString::number(hz, 'f', 3) + " Hz";
}
...
QString WaveformDisplay::formatVoltage(double volts) const
{
    ...
    if (absVolts == 0) return "0 V";
    else if (absVolts >= 1) return prefix + QString::number(absVolts, 'f', 3) + " V";
    else if (absVolts >= 1e-3) return prefix + QString::number(absVolts * 1e3, 'f', 3) + " mV";
    else return prefix + QString::number(absVolts * 1e6, 'f', 3) + " µV";
}
...
QPoint WaveformDisplay::scopeToScreen(const QPointF &scope) const
{
    ...
    int x = margin + static_cast<int>((scope.x() - m_xMin) * plotW / (m_xMax - m_xMin));
    int y = margin + static_cast<int>((m_yMax - scope.y()) * plotH / (m_yMax - m_yMin));
